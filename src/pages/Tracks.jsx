  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px', marginBottom: '40px' }}>
        {tracks.map((track) => (
          <div key={track.id} className="card" style={{ padding: '0', overflow: 'hidden', transition: 'transform 0.3s ease, box-shadow 0.3s ease', cursor: 'pointer', border: selectedTrack === track.id ? '2px solid #F4A261' : 'none' }}
            ...
            <div style={{ padding: '20px' }}>
              ...
              {track.isNew && (<div style={{ position: 'absolute', top: '20px', right: '20px', backgroundColor: '#b60232', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 'bold' }}>NEW</div>)}
              <Link to={track.path} className="btn-primary" style={{ width: '100%', textAlign: 'center', display: 'block' }}>View Track →</Link>
            </div>
          </div>
        ))}
      </div>
